...
class RedisService {
    ...
    socket_key(userId: Types.ObjectId) {
        return `user:Socket::${userId}`
    }
    async addSocket({ userId, socketId }: { userId: Types.ObjectId, socketId: string }) {
        return await this.client.sAdd(this.socket_key(userId), socketId)
    }
    async removeSocket({ userId, socketId }: { userId: Types.ObjectId, socketId: string }) {
        return await this.client.sRem(this.socket_key(userId), socketId)
    }
    async getSockets(userId: Types.ObjectId) {
        return await this.client.sMembers(this.socket_key(userId))
    }
    async hasSocket({ userId, socketId }: { userId: Types.ObjectId, socketId: string }) {
        return await this.client.sIsMember(this.socket_key(userId), socketId)
    }
